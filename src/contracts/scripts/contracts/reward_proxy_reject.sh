...
if [ "$network" == "" ] || [ "$contract" == "" ] || [ "$owner" == "" ] || [ "$rejected" == "" ]; then
  echo "Usage: $0 <network> <proxy-contract> <contract-owner> <rejected-contract>"
  exit 0
fi
...
params=$(ligo compile-parameter src/reward_proxy.ligo main "RejectContract((\"$rejected\": address))")
